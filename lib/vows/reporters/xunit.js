...
this.report = function (data) {
    ...
    case 'vow':
        ...
        case 'honored':
            ...
            break;
        case 'broken':
            var fail = tag('failure', {type: 'vows.event.broken', message: 'Broken test'}, false, cdata(event.exception.error));
            buffer.push(tag('testcase', {classname: curSubject, name: event.context + ': ' + event.title}, false, fail));
            break;
        case 'errored':
            var err = tag('error', {type: 'vows.event.errored', message: 'Errored test'}, false, cdata(event.exception.error));
